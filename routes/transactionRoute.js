...
module.exports = (router) => {
  ...
  console.log("called:")
    ...
    Transaction.find({ tripId: req.query.tripId, type: 'debit' }, (err, transactions) => {
        ...
        router.delete('/deletetx', (req, res) => {
          // console.log(req.body.tripId);
              
                // Save user to database
                Transaction.deleteOne({txId: req.query.transactionId},(err) => {
                  if(err){
                        res.json({ success: false, message: 'Could not delete  transaction. Error: ', err }); // Return error if not related to validation
                      } else {
                        res.json({ success: true, message: 'transaction deleted!!!' }); // Return success
                      }
                });
